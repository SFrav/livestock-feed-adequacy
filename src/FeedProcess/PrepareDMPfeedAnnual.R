...
#rasterOptions(tmpdir = "/exports/eddie/scratch/sfraval/")
...
#DEA300 <- projectRaster(DEA300, stDMP)
#DEA300 <- resample(DEA300, stDMP, method="ngb")
###Edits to LU
...
#stDMP <- reclassify(stDMP, c(-Inf, 0, 0)) 
...
#stLU$LUgrassShrub300 <- overlay(LUcrops300DEA, stLU$LUgrassShrub300, fun = function(diff, grass_shrub){ifelse(diff > 0 & grass_shrub >= diff, grass_shrub - diff, grass_shrub)})
#stLU$LUgrassShrub300 <- overlay(LUcrops300DEA, stLU$LUgrassShrub300, fun = function(diff, grass_shrub){ifelse(diff < 0 & grass_shrub + abs(diff) < 1, grass_shrub + abs(diff), grass_shrub)})
...
#iSPAMAnimalDigestCropFrac <- raster('SpatialData/inputs/SPAM2017/animal_digest_frac.tif')


#iSPAMAnimalDigestCropFrac <- resample(iSPAMAnimalDigestCropFrac, stDMP[[1]], method = "ngb")
#iSPAMAnimalDigestCropFrac <- extend(iSPAMAnimalDigestCropFrac, extent(stDMP[[1]]))
#iSPAMAnimalDigestCropFrac <- crop(iSPAMAnimalDigestCropFrac, extent(stDMP[[1]]))
#iSPAMAnimalDigestCropFrac <- mask(iSPAMAnimalDigestCropFrac, aoi)
...
#funGrowingGrass <- function(dmp, crops, grass, forest, shrub, nonprotected) {(dmp*9*grass*grassFrac)+(dmp*9*shrub*grassFrac*(1-browseScrubFrac))+(dmp*9*forest*grazeForestFrac*nonprotected)} #+ (dmp*9*crops*feedFrac*resFrac)}
...
#funGrowingGrassWet <- function(dmp, crops, grass, forest, shrub, shrubFrac, greenup, senesence, greenup2, senesence2, nonprotected) {ifelse((greenup <= datesDMPdiff[i] & senesence >= datesDMPdiff[i]) | (greenup2 <= datesDMPdiff[i] & senesence2 >= datesDMPdiff[i]), (dmp*9*grass*grassFracWet*(1-shrubFrac))+(dmp*9*shrub*grassFracWet*(1-shrubFrac))+(dmp*9*forest*grassFracWet*(1-shrubFrac)*nonprotected), NA) } #@feedFrac is the proportion of crops grown that have feedable residues - i.e. excluding coffee, tea, ect.
#funGrowingGrassDry <- function(dmp, crops, grass, forest, shrub, shrubFrac, greenup, senesence, greenup2, senesence2, nonprotected) {ifelse((greenup > datesDMPdiff[i]) | (senesence < datesDMPdiff[i] & senesence + 60 > datesDMPdiff[i]) | (senesence2 < datesDMPdiff[i]), (dmp*9*grass*grassFracDry*(1-shrubFrac))+(dmp*9*shrub*grassFracDry*(1-shrubFrac))+(dmp*9*forest*grassFracDry*(1-shrubFrac)*nonprotected), NA) } #@feedFrac is the proportion of crops grown that have feedable residues - i.e. excluding coffee, tea, ect.
#funGrowingBrowse <- function(dmp, crops, grass, forest, shrub, shrubFrac, nonprotected) {(dmp*9*shrub*shrubFrac*browseShrubFrac)+(dmp*9*grass*shrubFrac*browseShrubFrac)+(dmp*9*forest*nonprotected*shrubFrac*browseForestFrac)} 
##funGrowingCrops <- function(dmp, crops, greenup, senesence, feedFrac, resFrac, greenup2, senesence2) {ifelse((greenup <= datesDMPdiff[i] & senesence >= datesDMPdiff[i]) | (greenup2 <= datesDMPdiff[i] & senesence2 >= datesDMPdiff[i]), (dmp*9*crops*feedFrac*resFrac), NA) } #@feedFrac is the proportion of crops grown that have feedable residues - i.e. excluding coffee, tea, ect.


#funGrowingCrops <- function(dmp, crops, greenup, senesence, feedFrac, resFrac, greenup2, senesence2) {ifelse(((greenup - (365*5)) <= datesDMPdiff[i] & (senesence - (365*5)) >= datesDMPdiff[i]) | ((greenup2 - (365*5)) <= datesDMPdiff[i] & (senesence2 - (365*5)) >= datesDMPdiff[i]), (dmp*9*crops*feedFrac*resFrac), NA) } #@feedFrac is the proportion of crops grown that have feedable residues - i.e. excluding coffee, tea, ect.
#stDMP <- calc(stDMP, fun = function(DMP){ (DMP*9) })

#iDMP <- stack()
#iDMPCropGrowing <- stack()
